...
и теги инбаундов, поэтому выданные ссылки и привязки юзеров в панели не ломаются —
меняется только SNI (его надо обновить в ссылке у клиента). Занятый нашим же `xray`
порт 443 повторному прогону не мешает.
...
> **Про `www.microsoft.com`.** Он был дефолтом в этом репо и заменён на
> `www.apple.com`, потому что apple прошёл сквозной тест, а microsoft на том же
> стенде — нет. Считать это доказанным свойством домена нельзя: прогон был один,
> и переменная там была не одна. Важнее другое — **косвенные проверки такое не
> ловят**: порт слушает, `openssl s_client` снаружи отдаёт настоящий сертификат
> с `Verify return code: 0`, dest доступен по TLS1.3+H2, а туннель не работает.
> Меняя dest, всегда подтверждай результат `08-selftest.sh`, а не сертификатом.
